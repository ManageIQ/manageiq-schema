class AddFileDepotIdToMiqSchedule < ActiveRecord::Migration[4.2]
  ...
  def up
    ...
    say_with_time "Updating Schedules with file depots" do
      FileDepot.where(:resource_type => "MiqSchedule").each { |depot| depot.resource.update(:file_depot_id => depot.id) if depot.resource }
    end
  ...
  def down
    ...
    say_with_time "Updating Schedules with file depots" do
      MiqSchedule.all.each { |schedule| schedule.file_depot.update(:resource_type => "MiqSchedule", :resource_id => schedule.id) if schedule.file_depot }
    end
